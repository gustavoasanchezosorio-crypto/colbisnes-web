...
export default function CheckoutPage({ producto }: Props) {
  ...
  useEffect(() => {
    fetch("/api/tasa-usdt")
      .then(r => r.json())
      .then(d => { if (d.tasa) setTasa(d.tasa); });
  }, []);
  ...
  const handleContinuar = async () => {
    ...
    if (metodo === "online") {
      ...
      const res  = await fetch("/api/checkout/usdt", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ productoId: producto.id, tasaCOP: tasa }),
      });
      const data = await res.json();
  ...
  const cardClass = (m: MetodoPago) =>
    `w-full border-2 rounded-xl p-4 text-left transition-all ${
      metodo === m ? "border-yellow-400 bg-yellow-50" : "border-gray-200 hover:border-yellow-300"
    }`;
  ...
  return (
    <div className="max-w-lg mx-auto p-4 space-y-4">
      ...
      {/* ONLINE */}
      <button onClick={() => setMetodo("online")} className={cardClass("online")}>
        ...
        {metodo === "contraentrega" && (
          <div className="mt-3 pt-3 border-t border-green-200 space-y-1 text-sm">
            ...
            <div className="mt-3 bg-blue-50 border border-blue-200 rounded-lg p-3 text-xs text-blue-800 space-y-1">
              ...
              <p>2. Recibes un codigo secreto de 6 digitos por correo.</p>
      ...
      <button onClick={() => setMetodo("usdt")} className={`w-full border-2 rounded-xl p-4 text-left transition-all ${metodo === "usdt" ? "border-orange-400 bg-orange-50" : "border-gray-200 hover:border-orange-300"}`}>
        <div className="flex justify-between items-center">
          ...
          <div className="text-right">
            <p className="text-xs text-gray-400">Total</p>
            <p className="font-bold text-orange-600">{usdt.totalUSD} USDT</p>
          </div>
        </div>
        {metodo === "usdt" && (
          <div className="mt-3 pt-3 border-t border-orange-200 space-y-1 text-sm">
            ...
            <div className="mt-3 bg-orange-50 border border-orange-200 rounded-lg p-3 text-xs text-orange-900 space-y-1">
              ...
              <p>4. Colbisnes confirma el pago en la blockchain.</p>
              <p>5. El vendedor recibe su pago al confirmar entrega.</p>
      ...
      {metodo && (
        <button
          ...
          disabled={loading}
          className="w-full py-4 rounded-xl font-bold text-lg bg-yellow-400 hover:bg-yellow-500 transition disabled:opacity-50"
        >
