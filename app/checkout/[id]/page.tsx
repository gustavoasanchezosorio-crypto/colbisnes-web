...
export default function CheckoutPage() {
  ...
  const metodos = [
    { id: "online" as MetodoPago, icon: "💳", titulo: "Pago online seguro", sub: "Tarjeta · PSE · Nequi · Daviplata", badge: fmtPct(pctOnline), total: TEST_MODE ? "$50" : fmt(online.totalComprador + extras.extraTotal), desglose: [{ label: "Precio producto", val: fmt(online.precioBase) }, { label: TEST_MODE ? "Modo pruebas" : `Comision (${fmtPct(pctOnline)})`, val: TEST_MODE ? "$50" : fmt(online.comisionColbisnes) }, ...(TEST_MODE ? [] : desgloseExtrasCOP)], totalLabel: "Total a pagar", totalVal: TEST_MODE ? "$50" : fmt(online.totalComprador + extras.extraTotal), nota: ["Tu dinero queda protegido hasta confirmar la entrega.", notaDescuento].filter(Boolean).join(" ") },
    { id: "contraentrega" as MetodoPago, icon: "📦", titulo: "Contra entrega", sub: "Efectivo al recibir + reserva por Nequi", badge: fmtPct(pctContra), total: TEST_MODE ? "$50" : fmt(contra.totalComprador + extras.extraTotal), desglose: [{ label: "Precio producto", val: fmt(contra.precioBase) }, { label: TEST_MODE ? "Modo pruebas" : `Comision (${fmtPct(pctContra)})`, val: TEST_MODE ? "$50" : fmt(contra.comisionColbisnes) }, ...(TEST_MODE ? [] : desgloseExtrasCOP)], totalLabel: "Total al mensajero", totalVal: TEST_MODE ? "$50" : fmt(contra.totalComprador + extras.extraTotal), steps: ["Pagas por Nequi la comisión de reserva de Colbisnes (garantiza la compra — no es el pago del producto).", "Un administrador confirma tu pago manualmente; te avisamos apenas quede listo.", "El vendedor tiene 24 horas hábiles (8am-8pm) desde que se crea tu orden para despachar el producto.", "Mensajería entrega el producto — lo revisas al recibir.", "Confirmas la entrega en la app para liberar el pago al vendedor.", "Si el vendedor no despacha a tiempo, se bloquea su cuenta y gestionamos la devolución de tu comisión."], nota: ["La comisión de reserva se paga aparte por Nequi, antes del envío.", notaDescuento].filter(Boolean).join(" — ") },
      ...
      {showPopup && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(13,27,42,0.55)", backdropFilter: "blur(12px)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}
          ...
          <div className="glass" onClick={e => e.stopPropagation()}
            ...
            <p style={{ color: THEME.primary, fontSize: 16, fontWeight: 700, margin: "0 0 6px" }}>Solo se cobrarán $50 pesos</p>
        ...
        <div style={{ maxWidth: 480, margin: "0 auto" }}>
          ...
          <div className="glass" style={{ background: "rgba(255,255,255,0.14)", border: "1px solid rgba(255,255,255,0.25)", borderRadius: 18, padding: "16px 18px", textAlign: "center" }}>
            ...
            <div style={{ display: "flex", alignItems: "baseline", justifyContent: "center", gap: 12 }}>
              ...
              {TEST_MODE && <p style={{ color: "rgba(255,255,255,0.85)", fontSize: 14, fontWeight: 700, margin: 0 }}>→ cobro real: $50</p>}
            </div>
          </div>
        </div>
      </div>
