...
interface PremiumUser {
  ...
  premiumSolicitadoAt: string | null;
  premiumCedulaUrl: string | null;
  premiumComprobanteUrl: string | null;
}

export default function AdminPremiumPage() {
  const { data: session, status } = useSession();
  const router = useRouter();
  const [usuarios, setUsuarios] = useState<PremiumUser[]>([]);
  ...
  useEffect(() => {
    if (!session) return;
    setLoading(true);
    fetch(`/api/admin/premium?status=${filtro}`)
      .then((r) => r.json())
  ...
  async function aprobar(userId: string) {
    ...
    setMsg("");
    const r = await fetch("/api/premium/approve", {
      method: "PATCH",
    ...
    if (r.ok) {
      setMsg("✅ Badge premium aprobado y usuario notificado");
  ...
  async function rechazar(userId: string) {
    ...
    setActionLoading(null);
    if (r.ok) {
      setMsg("❌ Solicitud rechazada y usuario notificado");
      setUsuarios((prev) => prev.filter((u) => u.id !== userId));
    } else {
      setMsg("❌ " + d.error);
    }
    ...
    <div style={{ minHeight: "100vh", background: THEME.background, fontFamily: "system-ui, sans-serif" }}>
      <header style={{ background: `linear-gradient(135deg,${THEME.primaryLight},${THEME.primary} 52%,${THEME.primaryDark})`, padding: "0 24px", height: 64, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <span style={{ fontSize: 13, fontWeight: 700, color: "#fff", background: "rgba(255,255,255,0.15)", border: "1px solid rgba(255,255,255,0.3)", padding: "3px 10px", borderRadius: 20 }}>PREMIUM</span>
        </div>
      ...
      <main style={{ maxWidth: 900, margin: "32px auto", padding: "0 20px 80px" }}>
        <h1 style={{ fontSize: 22, fontWeight: 900, color: THEME.text, marginBottom: 20, textAlign: "center" }}>Verificación premium (badge)</h1>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
                  <div>
                    ...
                    <p style={{ fontSize: 12, color: THEME.muted, margin: 0 }}>
                      Solicitado: {u.premiumSolicitadoAt ? new Date(u.premiumSolicitadoAt).toLocaleString("es-CO") : "—"}
                    </p>
                  </div>
                  {filtro === "pending" && (
                    <div style={{ display: "flex", gap: 10 }}>
                      <button onClick={() => aprobar(u.id)} disabled={!!actionLoading} style={{ padding: "10px 22px", borderRadius: 12, background: "#10B981", color: "#fff", border: "none", fontWeight: 800, fontSize: 14, cursor: "pointer" }}>
                        {actionLoading === u.id + "_ok" ? "..." : "✅ Aprobar"}
                      </button>
                      <button onClick={() => rechazar(u.id)} disabled={!!actionLoading} style={{ padding: "10px 22px", borderRadius: 12, background: "#EF4444", color: "#fff", border: "none", fontWeight: 800, fontSize: 14, cursor: "pointer" }}>
                ...
                <div style={{ display: "flex", gap: 16 }}>
                  {u.premiumCedulaUrl && (
                    <div style={{ flex: 1.6 }}>
                      ...
                      <div style={{ borderRadius: 12, overflow: "hidden", cursor: "zoom-in", background: THEME.surfaceAlt, aspectRatio: "16/10", position: "relative" }} onClick={() => setLightbox(u.premiumCedulaUrl!)}>
                        <img src={u.premiumCedulaUrl} alt="Cédula" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                      </div>
                    </div>
                  )}
                  {u.premiumComprobanteUrl && (
                    <div style={{ flex: 1 }}>
                      <p style={{ fontSize: 11, fontWeight: 700, color: THEME.muted, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 8 }}>Comprobante de domicilio</p>
                      <div style={{ borderRadius: 12, overflow: "hidden", cursor: "zoom-in", background: THEME.surfaceAlt, aspectRatio: "1", position: "relative" }} onClick={() => setLightbox(u.premiumComprobanteUrl!)}>
                        <img src={u.premiumComprobanteUrl} alt="Comprobante de domicilio" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                      </div>
                    </div>
                  )}
                  {!u.premiumCedulaUrl && !u.premiumComprobanteUrl && (
      ...
      {lightbox && (
        <div onClick={() => setLightbox(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", cursor: "zoom-out" }}>
          <img src={lightbox} alt="Documento" style={{ maxWidth: "90vw", maxHeight: "90vh", borderRadius: 16, objectFit: "contain" }} />
        </div>
      )}
    </div>
  );
